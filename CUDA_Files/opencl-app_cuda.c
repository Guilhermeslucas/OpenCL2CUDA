...
int main( int argc, char* argv[] )
{
	...
	CUdevice device_id;           // device ID
	CUcontext context;               // context
	cl_command_queue queue;           // command queue
	CUmodule program;               // program
	CUfunction kernel;                 // kernel
	...
	err  = clSetKernelArg(kernel, 0, sizeof(CUdeviceptr), &d_a);
	err |= clSetKernelArg(kernel, 1, sizeof(CUdeviceptr), &d_b);
	err |= clSetKernelArg(kernel, 2, sizeof(CUdeviceptr), &d_c);
